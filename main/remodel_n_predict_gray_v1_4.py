...
model = keras.models.load_model(os.path.join(main_dir, "0.6776_model_bw_['A', 'B', 'C', 'D', 'E', 'F', 'H']__e-60_spe-60_vspe-60_bs-32_5L(128,64,32,16,8)_loss-0.975798.h5"))
...
while counter <= max_counter:
    ...
    if best_val_loss == val_loss_list[-1]:
        model_name = f'remodel_bw_{classes}__e-{counter}_spe-{spe}_vspe-{vs}_bs-{bs}_5L(128,64,32,16,8)_loss-{round(val_loss_list[-1],6)}.h5'
        model.save(os.path.join(main_dir, model_name))


    counter+=1
print(f'\n Final list was:\n {val_loss_list}')
model_name = f'end_of_remodel_bw_{classes}_e-{counter}_spe-{spe}_vspe-{vs}_bs-{bs}_5L(128,64,32,16,8)_loss-{round(val_loss_list[-1],6)}.h5'
model.save(os.path.join(main_dir, model_name))
...
model_name = f'remodel_bw_{classes}__e-{best_epoch + 1}_spe-{spe}_vspe-{vs}_bs-{bs}_5L(128,64,32,16,8)_loss-{round(best_val_loss, 6)}.h5'
