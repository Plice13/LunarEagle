#for given image path gives equatorial coordinations (Right ascension and Declination) of middle

from __future__ import print_function


#for calculating lenght of process
import time
...
#for coordination function
from astropy.io import fits


#client.py made as class
#need more adjustments but works
class API: 
    #!/usr/bin/env python3
    ...
    class Client(object):
        ...
        def send_request(self, service, args={}, file_args=None):
            ...
            try:
                f = urlopen(request)
                print('Current time is', datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'), 'from start is:', time.time()-start_time)
                print('Got reply HTTP status code:', f.status)
                txt = f.read()
                print('Got json:', txt)
                result = API.json2python(txt)
        ...
        def jobs_by_tag(self, tag, exact):
            ...
            return result
    def run_process(image_path, wcs_output_path):
        print("Running with args", sys.argv)
        parser = optparse.OptionParser()
        print('Parser je:', parser)

        parser.add_option('--server', dest='server', default=API.Client.default_url,
                        help='Set server base URL (eg, %default)')
        parser.add_option('--apikey', '-k', dest='apikey', default='xpulyjwkgllmjqxr',
        ...
        #SETTING
        opt.upload = image_path
        opt.wcs = wcs_output_path
        opt.crpix_center = True
        opt.center_ra = 301
        opt.center_dec = 19
        opt.radius = 8
        
        print('OPT A ARGS:', opt, 'LOL', args)
        print('takschvalne, 51:', opt.apikey)
...
#fits_info.py like function
def coordination(fits_file_path):
    # Open the FITS file
...
aimage_path = 'Pictures\Sip.JPG'
awcs_output_path = 'wcs_automatic.fits'

API.run_process(aimage_path,awcs_output_path)
ra_center, dec_center = coordination(awcs_output_path)
print('')
print(f'Center of ra is {ra_center} and center of dec is {dec_center}')
