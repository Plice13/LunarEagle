#plots the reference stars with its name (see other python programs) and brightest stars from Gemini and then connect some of them
...
def plot_photo(name_and_coordinations):
    for star in name_and_coordinations:
        x,y = star[1]
        x=-x
        plt.scatter(x, y, marker='o', s=8, c='blue')  # 'marker' sets the marker style, 's' sets the marker size
        plt.text(x, y, str(star[0]), fontsize=10, ha='center', va='center', c='orange')    # Customize the plot (optional)
...
def make_constelation(coordinates):
    for coord in coordinates:
        ...
        plt.text(x, y, str(coord[0]), fontsize=10, ha='center', va='center', c='green')    # Customize the plot (optional)
...
def plot_data_from_csv_and_coordinates(csv_file):
    # Read the CSV file into a DataFrame
    df = pd.read_csv(csv_file)
...
csv_file_path2 = r'fits\rdls.fits.csv'
...
name_and_constelation = get_star_name(csv_file_path)
plot_data_from_csv_and_coordinates(csv_file_path2)
make_lines(parky,constelation)
make_constelation(constelation)
plot_photo(name_and_constelation)
show()